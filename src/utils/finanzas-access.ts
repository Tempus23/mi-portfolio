import type { RuntimeEnv } from "@/utils/runtime-env";
...
function hasAccessAssertion(request: Request): boolean {
  ...
  return Boolean(jwt?.trim());
}

export function requireFinanzasAccess(request: Request, env?: RuntimeEnv): Response | null {
  ...
  if (!hasAccessAssertion(request)) {
    return jsonResponse(
      { error: "Acceso restringido. Inicia sesión mediante Cloudflare Access para usar finanzas." },
      401,
    );
  }

  const allowedEmails = normalizeEmailList(env?.FINANZAS_ALLOWED_EMAILS);

  if (allowedEmails.length === 0) {
    if (env?.FINANZAS_ALLOW_ALL_ACCESS === "true") return null;
    return jsonResponse(
      { error: "Configuración inválida: define FINANZAS_ALLOWED_EMAILS o habilita FINANZAS_ALLOW_ALL_ACCESS." },
      500,
    );
  }
  ...
  if (!allowedEmails.includes(accessEmail)) {
    return jsonResponse({ error: "Tu usuario no tiene permisos para acceder a finanzas." }, 403);
  }
